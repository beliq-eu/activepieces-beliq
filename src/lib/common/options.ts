...
// profile) are offered as extra generate targets beside the plain standards.
// A plain standard keeps its orthogonal Output / Factur-X-profile controls; a
...
export const STANDARD_OPTIONS: Option[] = [
  ...toOptions(LIVE_GENERATE_STANDARDS),
  ...PROFILE_PRESETS.map((p) => ({ label: p.label, value: p.id })),
];
